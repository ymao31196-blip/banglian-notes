...
> 开源模型更新（2026-06-7）：新增 Tyan-WP。当前已确认 WindFM 和 Tyan-WP 均属于公开代码与预训练权重的风电功率专用基础模型。
> 调研日期：2026-06-03/06-12 
...
截至 2026 年 6 月 7 日，不能再表述为“开源领域只有 WindFM”。目前已确认至少有两个公开代码和预训练权重的风电功率专用基础模型：WindFM 和 Tyan-WP。更准确的判断是：

> 目前公开可用的风电功率专用基础模型仍然很少，WindFM 和 Tyan-WP 是已确认的代表性模型；其他开源方案大多属于通用时间序列基础模型，或者需要针对单个场站重新训练的传统风电预测模型。
...
| Tyan-WP | 新发布的风电专用基础模型 | 跟踪并实验 | 融合历史功率、历史气象和场站静态信息，支持零样本概率预测 |
...
截至 2026 年 6 月 12 日，已确认至少有两个满足“风电功率专用、经过大规模预训练、公开代码和权重”条件的基础模型。

| 模型 | 发布时间 | 是否开源代码与权重 | 主要输入 | 主要特点 |
|---|---|---:|---|---|
| WindFM | 2025 年 9 月 | 是 | 历史功率、历史气象、时间戳 | 离散 token、Decoder-only Transformer、零样本概率预测 |
| Tyan-WP | 2026 年 6 月 7 日 | 是 | 历史功率、历史气象、时间戳、场站静态信息 | 场站静态嵌入、功率感知气象融合、稀疏专家编码、直接多步分位数预测 |

因此，不宜再使用以下绝对口径：

> 目前没有其他类似 WindFM 的风电功率专用开源基础模型。

建议改为：

> 目前公开可用的风电功率专用基础模型数量仍然很少，已确认的代表性模型包括 WindFM 和 Tyan-WP。其他开源方案大多是通用时间序列基础模型，或需要在单个场站数据上重新训练的传统风电预测模型。

#### Tyan-WP 的新增能力

Tyan-WP 是 2026 年 6 月新发布的超短期风电功率概率预测基础模型。它在大规模风电数据预训练基础上增加了两类面向风电业务的结构：

1. 场站静态嵌入：使用经纬度、地形类别和生态区域等信息描述不同场站的环境差异。
2. 功率感知气象融合：显式建模历史功率变化与历史气象协变量之间的关系。

相较 WindFM，Tyan-WP 更明确地考虑了复杂地形和场站差异，这对京津冀丘陵、山地场站具有较高研究价值。其模型通过稀疏专家编码器和多预测时长分位数预测头，直接输出不同提前量的概率预测。

Tyan-WP 已公开：

- 论文：<https://arxiv.org/abs/2606.08630>
- GitHub：<https://github.com/USTC-AI4EEE/Tyan-WP>
- Hugging Face 权重：<https://huggingface.co/USTC-CILab/Tyan-WP>

#### Tyan-WP 仍没有解决的问题

Tyan-WP 当前融合的是历史功率、历史气象观测和场站静态信息。论文实验为保持统一比较，关闭了未来天气输入，并将“融合数值天气预报或盘古、风乌等天气大模型预报，扩展至中短期风电功率预测”列为后续工作。

因此，Tyan-WP 不能被直接理解为已经具备以下完整能力：
...
> 开源领域已经出现 WindFM 和 Tyan-WP 两个风电功率专用基础模型，但仍缺少成熟开源的“风电专用基础模型 + 未来多源 NWP + 未来 10 天功率预测”完整方案。未来 10 天任务仍应以多源 NWP、场站订正和功率映射模型为主，并把风电基础模型作为迁移学习、概率预测、备用预测或模型融合成员。
...
> 经调研，WindFM 和 Tyan-WP 是当前值得关注的风电功率专用基础模型。它们与普通时间序列模型不同，主要价值在于利用大规模风电数据进行预训练，具备零样本迁移和概率预测方面的潜力。WindFM侧重离散token和自回归生成，Tyan-WP进一步加入场站静态信息和历史气象融合。但两者当前均不能直接替代面向未来10天的多源NWP功率预测系统。当前更稳妥的路线仍是先用LightGBM、XGBoost建立多源气象强基线，再将WindFM、Tyan-WP、TFT、PatchTST、NHITS等作为对照、迁移或融合模型。
...
10. 未来是否允许在企业服务器上直接训练或调用 WindFM、Tyan-WP、TFT、PatchTST、TimesFM、Chronos、Moirai、TTM 等深度模型或预训练模型？
...
- Tyan-WP GitHub：<https://github.com/USTC-AI4EEE/Tyan-WP>
- Tyan-WP Hugging Face 权重：<https://huggingface.co/USTC-CILab/Tyan-WP>
